...
SeaweedFS を使う object storage モード:

- `docker compose up -f docker-compose.object.yml`

object storage モードでは `runtask-ui.object.yaml` を使い、SeaweedFS の S3 endpoint に履歴インデックスと `runs/<runId>/...` を書き込みます。実行中は local staging を使い、完了時や worktree prune 後に object storage 側へ再同期します。
